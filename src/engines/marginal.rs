//! The `marginal` module handles scraping from Marginal Search.
...
impl SearchEngine for Marginal {
    async fn results(
        &self,
        query: &str,
        page: u32,
        ...
        let url = format!(
            ...
            encode(query.as_bytes()).to_string(),
            page * 10
        );
        ...
        let document: Html = Html::parse_document(
            &Marginal::fetch_html_from_upstream(self, &url, header_map, client).await?,
        );
